...
int main() {
  ...
  sound1.setBuffer(buf1);
	//vectors for keys
  freq = CONCERT_A * pow(2, 3.0/12.0);//vector for frq
  GuitarString gs2 = GuitarString(freq);//vectors fo fobjects
  sf::Sound sound2; 
  sf::SoundBuffer buf2;
...
/*
  ...
  std::vector<GuitarString> myGuitar(37); 
  std::vector<sf::Sound> mySound(37); 
  std::vector<sf::SoundBuffer> mySB(37);
  for(unsigned int i=0;i<=keyboard.length();i++){
    myGuitar[i] = GuitarString(440*pow(2,(i-24)/12));
    samples = makeSamplesFromString(myGuitar[i]);
    if (!mySB[i].loadFromSamples(&samples[0], samples.size(), 2, SAMPLES_PER_SEC))
      throw std::runtime_error("sf::SoundBuffer: failed to load from samples.");
    mySound[i].setBuffer(mySB[i]);
  }
*/
  while(window.isOpen()){
